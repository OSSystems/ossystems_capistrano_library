...
Capistrano::Configuration.instance(:must_exist).load do
  ...
  after  'deploy:first_code_deployed', 'deploy:assets:precompile'
  before 'deploy:assets:precompile',   'deploy:assets:update_asset_mtimes'
  before 'deploy:assets:precompile',   'deploy:assets:create_temp_dir'
  ...
  namespace :deploy do
    namespace :assets do
      ...
      desc <<-DESC
        [internal] Creates temporary directories and set them to be writable by the application user.
      DESC
      task :create_temp_dir, :roles => lambda { assets_role }, :except => { :no_release => true } do
        should_deploy_assets = application_config["should_deploy_assets"].nil? ? true : application_config["should_deploy_assets"]
        if should_deploy_assets
          cache_dir = File.join(latest_release, "/tmp/cache")
          sudo "mkdir -p #{cache_dir}"
          sudo "chown -R #{user}:#{application_user} #{cache_dir}"
      ...
      task :clean, :roles => lambda { assets_role }, :except => { :no_release => true } do
        should_deploy_assets = application_config["should_deploy_assets"].nil? ? true : application_config["should_deploy_assets"]
        if should_deploy_assets
          rvmsudo "#{rake} assets:clean", {:user => application_user, :path => latest_release, :env_vars => asset_env}
        end
      end
